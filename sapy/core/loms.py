...
CREATE_DEFAULT_LOMS = """
    ...
    ;""".format(
        OCCURRED_LIST_ID,
        OCCURRED_LIST_NAME,
        OCCURRED_LIST_VISIBLE,
        OCCURRED_LIST_COLOR,
        PLANNED_LIST_ID,
        PLANNED_LIST_NAME,
        PLANNED_LIST_VISIBLE,
        PLANNED_LIST_COLOR
        )
...
SET_TAB_VERSION = """
    INSERT INTO 'app_meta' ('key', 'value') VALUES ('lom_tab_version', ?)"""
...
class Lom(object):  # list of movements
    ...
    def get_moms(
        ...
        if _start_date is None and _end_date is None:
            ...
            cur.execute(
                GET_MOMS_3, (self.id, profile_id, _start_date, _end_date))
        ...
        for raw in cur.fetchall():
            ...
            mom = Mom(
                id=raw[0], value=raw[1],
                cause=raw[2], year=raw_year,
                month=raw_month, day=raw_day)
            mlist.append(mom)
        cur.close()
        return mlist
    ...
    def csv_import(self, csv_file):
        ...
        with open(str(csv_file.get_path()), 'r') as data_file:
            ...
            # TODO : add configuration option for delimiter
    ...
    def balance_per_day(self, profile_id, start_date=None, end_date=None):
        base_balance = self.balance(profile_id=profile_id, end_date=start_date)
        moms = self.get_moms(
            profile_id, start_date=start_date, end_date=end_date)
        ...
        while min_date <= max_date:
            ...
            moms = self.get_moms(
                profile_id, start_date=min_date, end_date=min_date)
...
def get_loms():
    ...
    for lom in cur.fetchall():
        if lom[2] == 1:
            llist.append(Lom(lom[0], lom[1], True, lom[3]))
        else:
            llist.append(Lom(lom[0], lom[1], False, lom[3]))
    cur.close()
    return llist
